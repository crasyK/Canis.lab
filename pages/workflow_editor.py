...
def get_single_data_value_by_name(state_data, node_name):
    for node in state_data.get('nodes', []):
        if node.get('state') == 'single_data' and node.get('name') == node_name:
...
def render_workflow_loader_dialog():
    """Simple workflow loader dialog"""
    with st.container():
        st.subheader("📂 Load Workflow")
        
        available_workflows = get_available_runs()
...
def render_single_data_creator():
    """Simplified single data creator"""
    with st.container():
        st.subheader("📊 Create Single Data")
        
        with st.form("single_data_form"):
            data_name = st.text_input("Name:", key="single_name")
            data_type = st.selectbox("Type:", ["string", "integer", "list", "json"], key="single_type")
            
            # Type-specific input
            if data_type == "string":
                data_value = st.text_input("Value:", key="single_value")
            elif data_type == "integer":
                data_value = st.number_input("Value:", value=0, key="single_int_value")
            elif data_type == "list":
                list_input = st.text_input("Items (comma-separated):", key="single_list_value")
                data_value = [item.strip() for item in list_input.split(',') if item.strip()] if list_input else []
            else:  # json
                json_input = st.text_area("JSON:", key="single_json_value")
                try:
                    data_value = json.loads(json_input) if json_input else {}
                except:
                    data_value = None
...
class SmartStepBuilder:
    ...
    def param_accepts_single_data(self, param_spec):
        ...
        is_single = list(param_spec.values())[0] == "single"
        return is_single
            ...
            if expected_types[0] == "file":
                ...
                with tab1:
                    if available_seed_files:
                        ...
                        data_value = selected_seed['path'] if selected_seed else None
                    else:
    ...
    def create_step_instance_for_pending(self, pending_step):
        ...
        try:
            ...
            StepClass.instances[pending_step['step_number']] = step_instance
...
def create_single_data_edges_from_state(state_data, flow_state):
    ...
    for steps in state_data.get('state_steps', []):
        step_inputs = steps.get('data', {}).get('in', {})
...
def add_pending_step(step_type, step_name, tool_name):
    ...
    try:
        ...
        StepClass.instances[next_step_number] = step_instance
...
with st.sidebar:
    ...
    if st.session_state.current_workflow:
        ...
        # Get the current workflow from the session state
        current_workflow = st.session_state.get('current_workflow', None)

        if not current_workflow:
            st.sidebar.info("No workflow loaded. Please load a workflow first using the 'Load Existing Workflow' section above.")
        else:
            from lib.directory_manager import dir_manager
            state_file_path = dir_manager.get_state_file_path(current_workflow)
            
            # Batch Cancellation Section
            with st.sidebar.expander("Cancel Running Batches", expanded=False):
                try:
                    uploaded_steps = get_uploaded_steps(str(state_file_path))
                    
                    if not uploaded_steps:
                        st.info("No running batches to cancel.")
                    else:
                        # Use session state to manage the UI for batch cancellation
                        if 'cancelling_batch' not in st.session_state:
                            st.session_state.cancelling_batch = False

                        if not st.session_state.cancelling_batch:
                            st.write(f"**{len(uploaded_steps)} running batch(es) found:**")
                            for step in uploaded_steps:
                                st.write(f"• {step['name']} ({step['tool_name']}) - {step['status']}")
                            
                            if st.button("🛑 Cancel a Batch"):
                                st.session_state.cancelling_batch = True
                                st.rerun()

                        if st.session_state.cancelling_batch:
                            st.warning("⚠️ You are about to cancel a running batch. This will stop the processing.")
                            
                            # Create options for the selectbox
                            batch_options = [f"{step['name']} (ID: {step['batch_id'][:8]}...)" for step in uploaded_steps]
                            step_names = [step['name'] for step in uploaded_steps]
                            
                            selected_option = st.selectbox(
                                "Select a batch to cancel:",
                                options=[None] + batch_options,
                                format_func=lambda x: "Choose a batch..." if x is None else x,
                                index=0
                            )
                            
                            selected_step_name = None
                            if selected_option:
                                # Find the corresponding step name
                                option_index = batch_options.index(selected_option)
                                selected_step_name = step_names[option_index]
                            ...
                            with col2:
                                if st.button("🛑 Confirm Cancellation", disabled=(not selected_step_name)):
                                    if selected_step_name:
                                        try:
                                            result = cancel_step_batch(str(state_file_path), selected_step_name)
                                            st.success(result)
                                            st.session_state.cancelling_batch = False
                                            
                                            # Clear cached state to force refresh
                                            keys_to_clear = ['flow_state', 'step_instances']
                                            for key in keys_to_clear:
                                                if key in st.session_state:
                                                    del st.session_state[key]
                                            
                                            # Reload the workflow
                                            state_data = load_workflow_state(current_workflow)
                    ...
                    st.error(f"Error loading running batches: {e}")

            # Step Deletion Section (existing code)
            with st.sidebar.expander("Delete saved Steps", expanded=False):
                try:
                    completed_steps = get_deletable_steps(str(state_file_path))

                    if not completed_steps:
                        st.info("There are no steps to delete.")
                    else:
                        # Use session state to manage the UI for deletion
                        if 'deleting_step' not in st.session_state:
                            st.session_state.deleting_step = False

                        if not st.session_state.deleting_step:
                            st.write(f"**{len(completed_steps)} saved step(s) found:**")
                            for step in completed_steps:
                                st.write(f"• {step}")
                            
                            if st.button("🗑️ Delete a Step"):
                                st.session_state.deleting_step = True
                                st.rerun()

                        if st.session_state.deleting_step:
                            st.warning("⚠️ You are about to delete a step. This action cannot be undone.")
                            
                            step_to_delete = st.selectbox(
                                "Select a step to delete:",
                                options=[None] + completed_steps,
                                format_func=lambda x: "Choose a step..." if x is None else x,
                                index=0
                            )
                            ...
                            with col2:
                                if st.button("🗑️ Confirm Deletion", disabled=(not step_to_delete)):
                                    if step_to_delete:
                                        try:
                                            result = delete_step(str(state_file_path), step_to_delete)
                                            st.success(result)
                                            st.session_state.deleting_step = False
                                            ...
                                            st.error(f"Failed to delete step: {e}")

                except Exception as e:
                    st.error(f"Error loading steps for deletion: {e}")
...
if st.session_state.current_workflow and st.session_state.flow_state:
    ...
    if st.session_state.pending_steps:
        ...
        st.info("💡 These steps are now visible in the diagram above, review them and click 'Execute' to run them.")
        ...
        for i, pending_step in enumerate(st.session_state.pending_steps):
            with st.container():
                ...
                if pending_step['tool'] == "Seed Data Generation":
                    seed_node_name = pending_step.get('connections', {}).get('seed_file_location').split('.')[1]
                    seed_value = get_single_data_value_by_name(current_state_data, seed_node_name)
                    
                    seed_file_path = seed_value
                        # Show enhanced preview with entry count
                    with st.expander("📋 Seed File Preview & Analysis", expanded=True):
                            seed_content, metadata = preview_seed_file(seed_file_path)
                            if seed_content:
