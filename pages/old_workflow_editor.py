...
from lib.state_managment import (
    ...
    get_step_deletion_preview, use_chip, delete_step_and_data, get_deletable_steps, delete_multiple_steps, clear_all_steps, recover_from_backup
...
def show_step_deletion_ui(state_file):
    """Show step deletion interface in Streamlit"""
    import streamlit as st
    
    st.subheader("🗑️ Step Management")
    
    try:
        deletable_steps = get_deletable_steps(state_file)
    except Exception as e:
        st.error(f"Error loading steps: {e}")
        return
    
    if not deletable_steps:
        st.info("No steps to delete")
        return
    
    # Single step deletion
    with st.expander("Delete Individual Step", expanded=False):
        step_options = [f"{step['name']} ({step['type']}) - {step['status']}" 
                       for step in deletable_steps]
        
        selected_step_display = st.selectbox("Select step to delete:", 
                                           ["Select a step..."] + step_options)
        
        if selected_step_display != "Select a step...":
            step_index = step_options.index(selected_step_display)
            step_info = deletable_steps[step_index]
            
            # Show step preview
            col1, col2 = st.columns(2)
            with col1:
                st.write(f"**Step:** {step_info['name']}")
                st.write(f"**Type:** {step_info['type']}")
                st.write(f"**Status:** {step_info['status']}")
            
            with col2:
                st.write(f"**Dependencies:** {step_info['dependencies']}")
            ...
            if st.button("🔍 Preview Deletion", key="preview_single"):
                try:
                    preview = get_step_deletion_preview(state_file, step_info['index'])
                    
                    st.write("**Deletion Preview:**")
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        st.write(f"Files to delete: {len(preview['files_to_delete'])}")
                        st.write(f"Markers to remove: {len(preview['markers_to_remove'])}")
                    
                    with col2:
                        st.write(f"Dependencies: {preview['dependencies']}")
                        st.write(f"Broken references: {len(preview['broken_references'])}")
                    
                    if preview['broken_references']:
                        st.warning("**Broken References:**")
                        for ref in preview['broken_references']:
                            st.write(f"- {ref}")
            ...
            col1, col2 = st.columns(2)
            with col1:
                delete_files = st.checkbox("Delete associated files", value=True, key="delete_files_single")
            with col2:
                create_backup = st.checkbox("Create backup", value=True, key="create_backup_single")
            
            # Delete button
            if st.button("🗑️ Delete Step", 
                        type="primary", 
                        disabled=not step_info['can_delete'],
                        key="delete_single"):
                try:
                    with st.spinner("Deleting step..."):
                        summary = delete_step_and_data(
                            state_file, step_info['index'], 
                            delete_files=delete_files, 
                            create_backup=create_backup
                        )
                    
                    st.success(f"✅ Successfully deleted step: {summary['step_name']}")
    ...
    with st.expander("Batch Operations", expanded=False):
        st.warning("⚠️ These operations affect multiple steps")
        
        # Multi-select for batch deletion
        st.write("**Select Multiple Steps for Deletion:**")
        selected_indices = []
        
        for i, step in enumerate(deletable_steps):
            if st.checkbox(f"{step['name']} ({step['type']})", 
                          key=f"batch_select_{i}",
                          disabled=not step['can_delete']):
                selected_indices.append(i)
        
        if selected_indices:
            st.write(f"Selected {len(selected_indices)} steps for deletion")
            
            col1, col2 = st.columns(2)
            with col1:
                batch_delete_files = st.checkbox("Delete associated files", value=True, key="delete_files_batch")
            with col2:
                batch_create_backup = st.checkbox("Create backup", value=True, key="create_backup_batch")
            
            if st.button("🗑️ Delete Selected Steps", type="secondary", key="delete_batch"):
                try:
                    with st.spinner(f"Deleting {len(selected_indices)} steps..."):
                        summaries = delete_multiple_steps(
                            state_file, selected_indices, 
                            delete_files=batch_delete_files, 
                            create_backup=batch_create_backup
                        )
                    
                    st.success(f"✅ Successfully deleted {len(summaries)} steps")
                    
                    # Show summaries
                    with st.expander("Deletion Summaries"):
                    ...
                    st.error(f"❌ Batch deletion failed: {e}")
        
        st.divider()
        
        # Clear all steps
        st.write("**Clear All Steps:**")
        col1, col2 = st.columns(2)
        
        with col1:
            if st.button("🗑️ Clear All Steps", type="secondary", key="clear_all"):
                try:
                    with st.spinner("Clearing all steps..."):
                        result = clear_all_steps(state_file)
                    
                    st.success(f"✅ Cleared workflow: {result['workflow_name']}")
        ...
        with col2:
            st.info("This will remove all steps and reset the workflow to empty state")

def show_recovery_ui(workflow_name):
    ...
    try:
        ...
        if not snapshots_dir.exists():
            st.info("No backups available for recovery")
            return
        ...
        for snapshot in snapshots:
            ...
            try:
                ...
                display_name = f"{timestamp.strftime('%Y-%m-%d %H:%M:%S')} ({snapshot.name})"
                snapshot_options.append((display_name, snapshot.stem))
            except:
                snapshot_options.append((snapshot.name, snapshot.stem))
        
        selected_snapshot = st.selectbox(
            "Select backup to restore:",
            ["Select a backup..."] + [opt[0] for opt in snapshot_options]
        )
        
        if selected_snapshot != "Select a backup...":
            snapshot_name = snapshot_options[[opt[0] for opt in snapshot_options].index(selected_snapshot)][1]
            
            st.warning("⚠️ This will overwrite the current workflow state!")
            
            if st.button("🔄 Restore from Backup", type="primary"):
                try:
                    with st.spinner("Restoring from backup..."):
                        restored_file = recover_from_backup(workflow_name, snapshot_name)
                    
                    st.success(f"✅ Successfully restored workflow from backup")
